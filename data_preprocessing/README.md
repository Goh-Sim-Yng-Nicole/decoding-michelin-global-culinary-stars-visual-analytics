<div align="center">
  ...
  <h1>🔬 Data Pre-Processing Pipeline</h1>
  <p><em>En-tire-ly Overrated? — Singapore Michelin Data</em></p>

  <p>A five-stage Python ETL pipeline that acquires, enriches, and integrates multiple data sources into Tableau-ready datasets.</p>
  ...
  ![Google Places](https://img.shields.io/badge/Google_Places_API-4285F4?style=flat-square&logo=googlemaps&logoColor=white)
...
| **`output/reviews.csv`** / **`.xlsx`** | 244,631 word-level token counts per restaurant × star rating (powers the website's interactive word cloud) |
...
┌──────────────────────────────────┐   ┌──────────────────────────────────┐
│  Stage 1                         │   │  Stage 1.1                       │
│  Core Data Enrichment            │   │  Non-Michelin Pipeline           │
│  (Michelin + API + SFA)          │   │  (Google API + Grid Sampling)    │
│  preprocess_restaurant_data.py   │   │  collect_restaurants.py          │
│  → michelin_restaurants.csv      │   │  → non-michelin_restaurants.csv  │
└──────────────┬───────────────────┘   └──────────────┬───────────────────┘
               │                                      │
               └────────────────┬─────────────────────┘
                                ▼
               ┌─────────────────────────────────────┐
               │  Integration Layer                   │
               │  Fuzzy Matching ≥ 85% + Manual       │
               │  Validation                          │
               │  → output/restaurants.csv (2,060)    │
               └────────────────┬────────────────────┘
                                │
               ┌────────────────▼────────────────────┐
               │  Stage 2: Review Acquisition         │
               ...
               └────────────────┬────────────────────┘
                                │
               ┌────────────────▼────────────────────┐
               │  Stage 3: NLI Sentiment Model        │
               │  sentiment_analysis.py               │
               │  (HuggingFace Zero-shot NLI)         │
               │  → reviews/*_review_level_           │
               │    sentiment.csv                     │
               └────────────────┬────────────────────┘
                                │
               ┌────────────────▼────────────────────┐
               ...
               │  preprocess_household_               │
               │  expenditure_data.py                 │
               │  (Tiers, Scores, Metrics)            │
               └────────────────┬────────────────────┘
                                │
               ┌────────────────▼────────────────────┐
               │  Final Output                        │
               │  master.xlsx — Tableau-Ready Dataset │
               │  (Restaurants + Affordability +      │
               │   Reviews + _Definitions)            │
               └─────────────────────────────────────┘
```
...
│   └── non-michelin_restaurants.csv             Enriched Non-Michelin (1,767 rows)
│
...
│
├── final_master/                                Final master datasets for Tableau
│   ├── master_online.xlsx                       For Tableau Public (web browser / online)
│   └── master_offline.xlsx                      For Tableau Desktop (offline only)
│
├── _backup/
...
| `non-michelin_restaurants.csv` | 1,767 | Enriched Non-Michelin restaurants (intermediary) |
...
flat table that powers the website's interactive review word cloud (Dashboard #2 — *What Diners Actually Say*).
...
**Website usage:** `reviews/wordcloud_data.csv` is pre-processed into `website/public/wordcloud_data.json` and served by the React website as an iframe embed. The word cloud renders per-restaurant word frequencies using `wordcloud2.js`, with a restaurant dropdown grouped by award tier.
...
| `Household_Affordability` | 2,060-row wide affordability table |
| `Reviews` | 244,631-row word token table (mirrors reviews.csv) |
| `_Definitions` | 7 reference tables: Restaurant columns, Affordability columns, Q1–Q5 Income Guide, Region by Postal Code, Sentiment model info, Affordability thresholds, Reviews columns |

---

### `final_master/` — Final Master Datasets

The `final_master/` folder contains the definitive versions of the master workbook used directly in the Tableau dashboards. There are two variants to match the two Tableau workbook formats:

| File | Use With | Description |
|------|----------|-------------|
| `master_online.xlsx` | Tableau Public (web browser) | Optimised for publishing to Tableau Public online |
| `master_offline.xlsx` | Tableau Desktop (offline) | For use with local Tableau Desktop only |

#### `Google Website Link` column behaviour

The `Google Website Link` column in the `Restaurants` sheet differs by restaurant type:

| Restaurant Type | `Google Website Link` value |
|----------------|----------------|
| **Michelin restaurants with own website** | The restaurant's own website URL |
| **Michelin restaurants without a website** | The Michelin Guide blog page for that restaurant |
| **Non-Michelin restaurants without a website** | Google Maps Embed API link (`maps/embed/v1/place?key=...`) |
...
Set API keys in a `.env` file in the project root:
```
GOOGLE_PLACES_API_KEY=your_places_api_key_here
GOOGLE_MAPS_EMBED_API_KEY=your_maps_embed_api_key_here
```
...
- **master.xlsx sheet structure:** All scripts that write `output/master.xlsx`
  ...
  Reviews, _Definitions) to preserve the multi-sheet structure. Using
